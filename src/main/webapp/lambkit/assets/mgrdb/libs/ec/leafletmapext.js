define(function (require) {
     ...
     * @constructor
     */
	function LeafletMapExt(map, ec) {
		this._init(map, ec);
	};
	//includes: [L.Mixin.Events],
	LeafletMapExt.prototype._echartsContainer = null;
	LeafletMapExt.prototype._map = null;
	LeafletMapExt.prototype._ec = null;
	LeafletMapExt.prototype._option = null;
	LeafletMapExt.prototype._geoCoord = [];
	LeafletMapExt.prototype._mapOffset = [0, 0];
	LeafletMapExt.prototype._delta = 0;
	LeafletMapExt.prototype._startTime = null;
	LeafletMapExt.prototype._lastMousePos = null;
	LeafletMapExt.prototype._bshow = false;
	LeafletMapExt.prototype._vid = false;

	LeafletMapExt.prototype._init = function (map, ec) {
		var self = this;
		self._map = map;
		...
		self.fadeOut = function(speed, callback) {
			$("#leaflet-map-ec-ext").fadeOut(speed,callback);
		};
		
		self.fadeIn = function(speed, callback) {
			$("#leaflet-map-ec-ext").fadeIn(speed,callback);
		};
		
		self.hide = function () {
			$("#leaflet-map-ec-ext").hide();
		};
		
		self.show = function () {
			$("#leaflet-map-ec-ext").show();
		};

		self.clear = function () {
			if (self._ec) self._ec.clear();
		};
		...
		self.remove = function () {
			if (self._bshow) {
				self._map.getPanes().overlayPane.removeChild(self._echartsContainer);
				self._bshow = false;
			}
		};
		...
		self.add = function () {
			if (!self._bshow) {
				...
				div.style.top = 0;
				div.style.left = 0;
				self._map.getPanes().overlayPane.appendChild(div);
			}
			self._bshow = true;
		};
		...
		self.setOption = function (option, notMerge) {
			...
			for (var i = 0, item; item = series[i++]; ) {
				if (item.type == "map") {
						...
						if (geoCoord) {
							//console.log("coord: " + geoCoord[0] + ", " + geoCoord[1]);
							var pos = self.geoCoord2Pixel(geoCoord);
							center[0] = pos[0];// - self._mapOffset[0];
							center[1] = pos[1];// - self._mapOffset[1];
          ...
          if (geoCoord) {
            for (var k in geoCoord) {
              self._geoCoord[k] = geoCoord[k];
         		...
         		if (item.type == "pie") {
         			var pradius = item.pradius;
         			var radius = item.radius;
         			var piesize = maplevel4size[0];
					var sizeScale = maplevel4size[1];
					var zoom = parseInt(map.getZoom());//触摸屏缩放时，zoom非整数
					if(zoom > maplevel4size.length || zoom===maplevel4size.length) zoom = maplevel4size.length;
					piesize = maplevel4size[zoom];
					if(isArray(radius)) {
         				item.radius[1] = radius[1] - pradius + piesize;
         				if(item.radius[0]!=0) item.radius[0] = item.radius[1] * 0.5;
         			} else {
         				item.radius = radius - pradius + piesize;
         			}
         			item.pradius = piesize;
         		}
			}
		...
		self._bindEvent = function () {
			self._map.on('resize', function(){
				//lamb 20160718 添加容器resize事件
				var mapsize = self._map.getSize();
				self._echartsContainer.style.width = mapsize.x + 'px';
				self._echartsContainer.style.height = mapsize.y + 'px';
				if (self._ec) self._ec.resize();
			});
			//self._map.on('movestart', _moveHandler('movestart'));
			...
			self._ec.getZrender().on('mousewheel', function (e) {
				...
				if(zoom >= map.getMaxZoom() && delta > 0) return;
				if(zoom <= map.getMinZoom() && delta < 0) return;
				self.clear(); //在mousewheel的时候清除echarts内容
				/*
		...
		/*
		self._unbindEvent = function () {
			self._map.off('move', _moveHandler('moving'));
			self._map.off('moveend', _moveHandler('moveend'));
			self._map.off('zoomstart', function () {
				if (self._ec)  self._ec.clear();
			}); //去掉zoomstart事件
			self._map.off('zoomend', _zoomChangeHandler);
			self._ec.getZrender().un('mousewheel', function (e) {
				if (self._ec)  self._ec.clear(); //在mousewheel的时候清除echarts内容
				self._lastMousePos = self._map.mouseEventToContainerPoint(e.event);
				var delta = L.DomEvent.getWheelDelta(e.event);
				var map = self._map,
				zoom = map.getZoom();
...
*/
		...
		function _moveHandler(type) {
			return function () {
				...
				self._echartsContainer.style.top = self._mapOffset[1] + 'px';
				*/
				if (self._ec) self.setOption(self._option);
				//_fireEvent(type);
				if (type == 'movestart') {
					//self.clear();
				}
