...
import com.lambkit.web.controller.tache.TacheValidator;
...
public class CommonController extends BaseController {
	...
	public void list()
	{
		...
		setAttr("rolename", temp); 
		if(temp.equals("member")){
			UpmsRole m = UpmsRole.service().dao().findFirst("select * from upms_role where name='member'");
			sql +=" and roleid='"+m.getRoleId()+"' ";
		}
	...
	*/
	...
	private String getSearchSQL(MgrTable tbc) { 
		return " from " + tbc.getName() + " " + getThisSearchWhereSQL(tbc, "");
	} 
	...
	public void save() {
		...
		if(idname.equals("fldid")||idname.equals("tbid")){
		...
		for(int i=0; i<flds.size(); i++) {
			...
			{  
				...
				if(fld.getDatatype().startsWith("float") || fld.getDatatype().startsWith("double")
					...
					m.set(fld.getName(), getParaToLong("model."+fld.getName())); 
				} else if(fld.getDatatype().startsWith("datetime") || fld.getDatatype().startsWith("timestamp")) { 
	...
	public void update() {
		...
		if(idname.equals("fldid")){
			m.set("editorder", Integer.parseInt(m.get("editorder").toString()));
		}
		m.set(tbc.getModel().getPrimaryKey(), Integer.parseInt(m.get(tbc.getModel().getPrimaryKey().toString())+""));
